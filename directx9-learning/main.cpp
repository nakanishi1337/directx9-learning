...
void DisplayMesh(FbxNode* node, FbxImporter* importer) {
	...
	int frame = endFrame - startFrame; //フレーム数
	...
	for (int i = 0; i < SkinNumber; ++i) {// スキンごとにループ
		...
		for (int j = 0; j < frame; ++j) {// フレームごとにループ
			for (int k = 0; k < clusterNum; ++k) {// ボーンごとにループ
	...
	//スキンメッシュ用頂点取り出し//////////////////////////////////////////////////////////////////
	double** WEIGHT = new double* [positionNum];
	...
	double** BONEID = new double* [positionNum];
	for (int i = 0; i < positionNum; i++) {
	...
	for (int i = 0; i < SkinNumber; ++i) {
		// i番目のスキンを取得
		//FbxSkin* skin = GetDeformer(i, FbxDeformer::eSkin);
		FbxSkin* skin = static_cast<FbxSkin*>(mesh->GetDeformer(i, FbxDeformer::eSkin));

		// クラスタ(ボーンのこと？)の数を取得
		int clusterNum = skin->GetClusterCount();

		for (int fff = 0; fff < positionNum; ++fff) {
			//std::cout << fff << "番目の頂点/////////////" << std::endl;
			for (int j = 0; j < clusterNum; ++j) {
				// j番目のクラスタ(ボーン)を取得
				FbxCluster* cluster = skin->GetCluster(j);
...
void DisplayContent(FbxNode* node, FbxImporter* importer)
{
	...
	{
		DisplayContent(node->GetChild(i), importer);
	}
}


//ノード辿り
void DisplayContent(FbxScene* scene, FbxImporter* importer)
{
	//ルートノード
	FbxNode* node = scene->GetRootNode();

	//DisplayContent(node)にすべてのnodeを渡す
	for (int i = 0; i < node->GetChildCount(); i++)
	{
		DisplayContent(node->GetChild(i), importer);
	}
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow){
 ...
 //ウィンドウの初期位置とサイズを決める
	...
	if (importer->Initialize(filename, -1, manager->GetIOSettings()) == false) {
		return -1;
	}
	//sceneにfbxの情報を入れる
	...
	while (TRUE) {
		...
		if (NowFrame == 155) { NowFrame = 0; }
		NowFrame += 1;
		for (int i = 0; i < BoneNumber; i++) {
			combMatAry[i] = /*INVinitMat[i]*/ FrameMat[NowFrame][i];//何フレーム目かの行列
		}
		...
		//g_pD3DDev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, PolygonNumber, &MyVertex, sizeof(CUSTOMVERTEX));
